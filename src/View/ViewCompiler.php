    ...
    protected function compileIncludes(string $content): string
{
    return preg_replace_callback(
        ...
        function ($matches) {
            ...
            // Use the view engine's render method instead of direct include
            return "<?php echo \$this->view->render('{$view}', array_merge(\$__data ?? [], {$data})); ?>";
        },
        $content
    );
}
    ...
    protected function compileSections(string $content): string
    {
        ...
        $content = preg_replace_callback(
            '/\@extends\s*\([\'"](.+?)[\'"]\)/',
            function($matches) {
                return "<?php \$__extends = '" . $matches[1] . "'; ?>";
            },
            $content
        );
        ...
        $content = preg_replace_callback(
            '/\@section\s*\([\'"](.+?)[\'"]\)/',
            function($matches) {
                return "<?php \$__currentSection = '" . $matches[1] . "'; ob_start(); ?>";
            },
            $content
        );
        ...
        $content = preg_replace_callback(
            '/\@endsection\b/',
            function($matches) {
                return "<?php if (isset(\$__currentSection)) { \$__sections[\$__currentSection] = ob_get_clean(); } ?>";
            },
            $content
        );
        ...
        $content = preg_replace_callback(
            ...
            function($matches) {
                $section = $matches[1];
                $default = $matches[2] ?? '';
                return "<?php echo isset(\$__sections['{$section}']) ? \$__sections['{$section}'] : '{$default}'; ?>";
            },
            $content
        );
    ...
    {
        return preg_replace('/\@csrf\b/', '<?php echo csrf_field(); ?>', $content);
    }
